...
This workshop aims to set up a toolbox to model large and complex molecules which contain heavy atoms in "realistic" environments.

The computer simulations of such ensembles are not easy since they require the model that includes the **relativistic effects** (scalar and spin-orbit), has a good description of **electron correlation**, and, finally, should account for the presence of an environment (**environmental effects**).

More advanced quantum chemistry methods which can encompass the relativistic and electron correlation effects are computationally costly, therefore cannot be used to a molecule and its environment treated as a whole system.

A promising alternative is the embedding approach, i.e., dividing the whole system into subsystems and treating them separately by the best (and tailored for them) quantum chemistry models.
An example of such a method is the **frozen density embedding (FDE)**, on which we focus in this workshop.

We will discuss the modeling of the **electronic structure** of molecules and their **molecular response properties** - primarily those related to observables in various molecular spectroscopies.
...
If the particle in question is the electron, then one needs quantum mechanical formulations, as they capture the electron's dual nature (particle-wave duality).

Nuclei are much heavier and slower than electrons and can be treated as classical particles. In our studies, we will primarily focus on electrons - on predicting **the electronic structure** of molecular systems.


However, in the quantum realm, one can only calculate the probability of finding the particle (the electron) at a certain time in a given position.
...
- the Schrodinger equation describes the "nonrelativistic" particle, while the Dirac equation describes the "relativistic" particle
  ...
  (1) one difference between the two equations stems from the Lorentz factor
    ...
    - if the particle moves with a speed close to the speed of light, then $\gamma > 1$ - requires a "relativistic" description
...
- we then choose the approximation for the wave function:
  ...
  - a choice to use one such determinant as a wave function is made for instance in the **Hartree-Fock (HF)** method,

  - a wave function describes how the movement of one electron affects the movement of another electron (**the electron correlation**),
...
- the Schrodinger/Dirac equations are solved in an iterative procedure, meaning that the wave function (i.e., the molecular orbitals) is optimized self-consistently under the conditions imposed by all approximations we introduced on the way.


Thus far, we discussed the basics of the **wave function theory (WFT)** models. These models are expensive - their computational cost (see below) depends on the number of electrons. For instance, the H2O molecule has 10 electrons, each described with 3 Cartesian coordinates (and one spin coordinate). Its wave function depends on $3\*10$ spatial and $1\*10$ spin variables. To simplify this, we can instead consider describing the system through its **electron density**, $\rho(\vec{r})$, which depends on the 3 Cartesian coordinates only (for any system, irrespective of its size). This choice stands as a basis for methods of the **density functional theory (DFT)**. Instead of constructing a wave function in DFT, one needs to choose the so-called **exchange-correlation functional**.
...
- the components of the quantum chemistry model:
  - how is one electron described:
    - "the basis set"
    - examples: **Gaussian sets** (e.g., cc-pVDZ, dyall.cv2z, ANO-RCC, ...)
  - how are many electrons described
    - "the method"
    - examples: **Hartree-Fock, Coupled-Cluster methods, DFT, ...**
  - how is physics described (particle motion and interactions):
    - the Hamiltonian
    - examples: **Dirac-Coulomb Hamiltonian, ZORA Hamiltonian, nonrelativistic Hamiltonian, ...**

- the better description of each = the more accurate model = the higher cost of calculations:
  - the Jaccob's ladder of chemical accuracy:
  ...
  - the **computational cost** of WFT/DFT methods is $xN^y$, where:
    ...
    - for instance, for the calculations on the H2O molecule, performed with the DC/HF/cc-pVDZ model[^1], and with no cost reduction (e.g., due to symmetries), we can evaluate the cost $4\*131^4$ (see "DIRAC-first-steps" lesson)),
